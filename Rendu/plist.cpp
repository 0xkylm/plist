...
void WalkOnProcess(int process_pid = 0) {
    ...
    printf("PID\tNAME\t\t\t\tTHREADS\t\tMEMORY USAGE\tELAPSED TIME\n");

    int i = 0;
    if (Process32First(hProcessSnap, &pe32)) {
        do {

            if (process_pid != 0 && process_pid != pe32.th32ProcessID)
                continue;
                                    ...
                                    //   ReadProcessMemory(CHANDLE, &idk.Reserved2, &state, sizeof(state), nullptr);
                                        ...
                                        // printf("from var:: %i\n", state);
                                            ...
                                            case 4:
                                                ...
                                                break;
                                            case 5:
                                                printf("Terminated"); //Tous marqu� terminated ?????????????????????????
                                                break;
                                            case 6:
                                                printf("Waiting");
                                                break;
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        if(!strcmp(argv[1], "-p") && argc > 2) {
            ...
            printf("-p <pid> : return info about the process\n");
            return 0;
        }
    } 
